...
function MovieDetails() {
        ...
        <div className={`bg-black/70 border border-white/20 rounded-2xl backdrop-blur-md ${compact ? "p-3" : "p-4"}`}>
            <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-1.5">
                    <button onClick={onPrevCard} className="text-white bg-violet-600/80 hover:bg-violet-500 rounded-full w-8 h-8 flex items-center justify-center transition text-sm">←</button>
                    <button onClick={onNextCard} className="text-white bg-violet-600/80 hover:bg-violet-500 rounded-full w-8 h-8 flex items-center justify-center transition text-sm">→</button>
                </div>
            </div>
            <div className={`relative ${compact ? "h-36" : "h-52"}`}>
                {factsCards.map((card, index) => {
                    ...
                    return (
                        <div key={card.label} className={`absolute top-0 left-0 rounded-xl border border-violet-400/35 bg-gradient-to-br from-gray-900/95 via-gray-800/90 to-slate-900/95 p-3 shadow-2xl transition-all duration-300 ${sizeClass} ${opacityClass} ${translate}`} style={{ zIndex }}>
                            <div className="flex items-center gap-2 text-sm text-violet-300 mb-1.5">
                                <span>{card.icon}</span>
                                <span className="font-semibold">{card.label}</span>
                            </div>
                            <p className={`text-white font-semibold ${compact ? "text-sm" : "text-base md:text-lg"}`}>{card.value || "N/A"}</p>
                        </div>
                    );
                })}
            </div>
    ...
    return (
        <div className="bg-black text-white min-h-screen">
            {toast && (
                <div className={`fixed top-4 left-4 right-4 sm:left-auto sm:top-6 sm:right-6 sm:w-auto z-[100] px-5 py-3 rounded-xl text-white text-sm font-semibold shadow-2xl border ${toast.type === "error" ? "bg-red-900/90 border-red-600" : "bg-violet-900/90 border-violet-500"}`}>
                    {toast.message}
                </div>
            )}
            ...
            <div className="lg:hidden">
                <div className="relative h-52 sm:h-64 bg-cover bg-center" style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original${movie.backdrop_path})` }}>
                    <div className="absolute inset-0 bg-gradient-to-b from-black/30 via-black/50 to-black" />
                </div>
                <div className="px-4 pt-2 pb-4">
                    <h1 className="text-2xl font-bold mb-2">{movie.title}</h1>
                    <div className="inline-flex items-center gap-1.5 bg-black/60 border border-violet-900/40 rounded-lg px-3 py-1.5 text-sm mb-4">⭐ {movie.vote_average?.toFixed(1)}</div>
                    <p className="text-gray-300 text-sm mb-5">{movie.overview}</p>
                    <FactsCarousel compact={true} />
                </div>
            </div>

            {/* DESKTOP HERO */}
            <div className="hidden lg:block relative h-[60vh] bg-cover bg-center" style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original${movie.backdrop_path})` }}>
                ...
                <div className="relative z-10 h-full p-8 flex items-end gap-6">
                    <div className="bg-black/60 p-6 rounded-xl max-w-3xl">
                        ...
                        <div className="bg-black/50 rounded-md p-2 inline-flex">⭐ {movie.vote_average?.toFixed(1)}</div>
                    </div>
                    <div className="ml-auto w-96 shrink-0"><FactsCarousel compact={false} /></div>
                </div>
            </div>
            ...
            <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3">
                <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-sm font-semibold shadow-lg transition-all flex items-center gap-2 ${inWatchlist ? "bg-green-600 border border-green-400 text-white" : "bg-violet-600 text-white hover:bg-violet-500"}`}
                >
                ...
                <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-sm font-semibold shadow-lg transition-all flex items-center gap-2 ${inFavorites ? "bg-pink-600 border border-pink-400 text-white" : "bg-pink-700 text-white hover:bg-pink-600"}`}
                >
            ...
            <div className="px-4 lg:px-10 py-10">
                ...
                <div className="lg:hidden flex items-center gap-2">
                    <button onClick={prevCast} className="w-8 h-8 rounded-full bg-violet-900/40 text-white">◀</button>
                    <div className="grid grid-cols-3 gap-3 flex-1">
                        {displayedCast.map((actor) => <CastCard key={actor.id} actor={actor} />)}
                    </div>
                    <button onClick={nextCast} className="w-8 h-8 rounded-full bg-violet-900/40 text-white">▶</button>
                </div>
                <div className="hidden lg:grid grid-cols-5 gap-4">
                    {movie.credits?.cast?.slice(0, 10).map((actor) => <CastCard key={actor.id} actor={actor} />)}
                </div>
            </div>

            <ReviewSection movieId={movie.id} movieTitle={movie.title} posterPath={movie.poster_path} />
        </div>
    );
}
...
function CastCard({ actor }) {
    return (
        <div className="group overflow-hidden rounded-2xl border border-violet-400/40 bg-gradient-to-br from-violet-900/80 p-1">
            <div className="relative aspect-[2/3] overflow-hidden rounded-xl">
                <img className="h-full w-full object-cover" src={actor.profile_path ? `https://image.tmdb.org/t/p/w300${actor.profile_path}` : "https://via.placeholder.com/300x450?text=No+Photo"} alt={actor.name} />
            </div>
